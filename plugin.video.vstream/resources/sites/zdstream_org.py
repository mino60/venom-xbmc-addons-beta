...
def load():
    ...
    oOutputParameterHandler = cOutputParameterHandler()
    oOutputParameterHandler.addParameter('siteUrl', 'http://venom')
    oGui.addDir(SITE_IDENTIFIER, 'showQlt', 'Films Qualit̩s', 'films.png', oOutputParameterHandler)
...
def showMovies(sSearch = ''):
    ...
    sPattern = '</small></div></div><div.*?class="moviefilm"><a.*?href="([^<]+)">.+?<img.*?src="([^<]+)" alt="(.+?)" height="125px" width="119px" /></a><div'
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            sThumb = aEntry[1] + '|Referer=http://zdstream.org/'
            ...
            if '/series-tv' in sUrl or '/series-tv' in aEntry[0]:
                oGui.addTV(SITE_IDENTIFIER, 'showSeries', aEntry[2],'', sThumb, '', oOutputParameterHandler)
            else:
                oGui.addMovie(SITE_IDENTIFIER, 'showLinks', aEntry[2], sThumb, sThumb, '', oOutputParameterHandler)          
